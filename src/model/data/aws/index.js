...
async function readFragment(ownerId, id) {
    logger.info(
        ...
        'aws/readFragment: Attempting to read fragment metadata'
    );
...
// Writes a fragment's data buffer to an S3 Object in a Bucket
...
async function writeFragmentData(ownerId, id, data) {
    ...
    try {
        ...
        logger.info({ res: res.$metadata }, 'S3 File Upload was successful');
    } catch (err) {
...
// Reads a fragment's data buffer from S3 and returns (Promise<Buffer>)
...
async function deleteFragment(ownerId, id) {
    logger.info(`model/data/aws: Attempting to deleteFragment`);
    ...
    try {
        ...
        logger.info(`Fragment ${id} deleted from S3 bucket ${process.env.AWS_S3_BUCKET_NAME}`);
        ...
        logger.info(
            `Fragment ${id} deleted from DynamoDB table ${process.env.AWS_DYNAMODB_TABLE_NAME}`
        );
    } catch (error) {
        logger.error(`Error deleting fragment metadata from DynamoDB: ${error.message}`);
        throw error;
    }
}
